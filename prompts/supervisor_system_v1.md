...
2. Else if `memory_summary` is empty, route to `Memory_Node`.
3. Else if `enable_fix_execution` is true and `fix_execution_result` is empty, route to `Execute_Fix_Node`.
...
- `next_worker`: one of `Topology_Node`, `Memory_Node`, `Execute_Fix_Node`, `FINISH`.
